FROM nvidia/cuda:12.8.1-cudnn-runtime-ubuntu24.04 AS build
...
ENV UV_COMPILE_BYTECODE=1 UV_LINK_MODE=copy UV_PYTHON=/usr/bin/python3.12
WORKDIR /app
...
RUN uv sync --frozen --no-dev --no-install-workspace --package atarang-worker --extra cuda
