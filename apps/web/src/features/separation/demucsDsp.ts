...
function reflectPad(signal: Float32Array, left: number, right: number) {
  const output = new Float32Array(left + signal.length + right);
...
function stft(signal: Float32Array) {
  ...
  const imaginary = new Float32Array(frames * bins);
  const input = new Array<number>(DEMUCS_FFT_SIZE);
  const spectrum = fft.createComplexArray();
  const scale = 1 / Math.sqrt(DEMUCS_FFT_SIZE);
  for (let frame = 0; frame < frames; frame++) {
    const offset = frame * DEMUCS_HOP_SIZE;
    for (let index = 0; index < DEMUCS_FFT_SIZE; index++) input[index] = signal[offset + index]! * window[index]!;
    fft.realTransform(spectrum, input);
    for (let bin = 0; bin < bins; bin++) {
      real[frame * bins + bin] = spectrum[bin * 2]! * scale;
      imaginary[frame * bins + bin] = spectrum[bin * 2 + 1]! * scale;
    }
  }
...
export function prepareDemucsInput(left: Float32Array, right: Float32Array) {
  const paddedLeft = new Float32Array(DEMUCS_SEGMENT_FRAMES);
  const paddedRight = new Float32Array(DEMUCS_SEGMENT_FRAMES);
  ...
  const leftSpectrum = stft(reflectPad(reflectPad(paddedLeft, side, rightPad), center, center));
  const rightSpectrum = stft(reflectPad(reflectPad(paddedRight, side, rightPad), center, center));
  const mag = new Float32Array(4 * DEMUCS_SPEC_BINS * DEMUCS_SPEC_FRAMES);
  ...
  const mix = new Float32Array(2 * DEMUCS_SEGMENT_FRAMES);
  mix.set(paddedLeft);
...
export function demucsFrequencyToTime(freq: Float32Array) {
  const stems = Array.from({ length: 4 }, () => ({ left: new Float32Array(DEMUCS_SEGMENT_FRAMES), right: new Float32Array(DEMUCS_SEGMENT_FRAMES) }));
  ...
  for (let stem = 0; stem < 4; stem++) {
    for (let channel = 0; channel < 2; channel++) {
      ...
      const imaginary = new Float32Array(paddedFrames * paddedBins);
      for (let frame = 0; frame < DEMUCS_SPEC_FRAMES; frame++) {
        for (let bin = 0; bin < DEMUCS_SPEC_BINS; bin++) {
...
export function combineDemucsBranches(freq: Float32Array, time: Float32Array): StereoStem[] {
  const frequency = demucsFrequencyToTime(freq);
  return frequency.map((stem, stemIndex) => {
    ...
    for (let frame = 0; frame < DEMUCS_SEGMENT_FRAMES; frame++) {
      left[frame] = stem.left[frame]! + time[base + frame]!;
      right[frame] = stem.right[frame]! + time[base + DEMUCS_SEGMENT_FRAMES + frame]!;
    }
    return { left, right };
  });
}
