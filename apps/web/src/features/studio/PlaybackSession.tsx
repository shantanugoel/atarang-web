...
export function PlaybackSessionProvider({ children }: { children: ReactNode }) {
  ...
  return <Context.Provider value={{ original, playback, waveform: waveform.waveform, waveformStatus: waveform.status, beatGrid: beats.grid, adjustTempo: beats.adjustTempo, separation, started }}>{children}</Context.Provider>;
}
