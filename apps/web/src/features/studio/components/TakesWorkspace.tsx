...
import{downloadTakeAsset,exportPerformanceMp3,previewPerformanceFiles}from"../../recording/exportPerformance";
...
export function TakesWorkspace({originalId}:{originalId?:string}){
  ...
  // LAME runs around five times realtime, so a long take leaves the button
  // silent for the better part of a minute unless it says what it is doing.
  const[encoding,setEncoding]=useState<string>();
  const exportMix=(take:PerformanceManifestV1)=>{setEncoding(take.performanceId);void exportPerformanceMp3(take).finally(()=>setEncoding(undefined))};
