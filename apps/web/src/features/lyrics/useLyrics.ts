...
import {getLyrics,putLyrics} from "../../storage/repositories";
...
export function useLyrics(originalId?:string){
  ...
  useEffect(()=>{let active=true;if(!originalId){setDocument(null);latest.current=null;return}setDocument(undefined);void getLyrics(originalId).then(record=>{if(!active)return;let value:LyricsDocumentV1|null=null;try{if(record){assertLyricsDocument(record.document);value=record.document;createdAt.current=record.createdAt}}catch{/* Damaged generated lyrics read as missing. */}latest.current=value;setDocument(value)});const hide=()=>flush();window.addEventListener("pagehide",hide);return()=>{active=false;flush();window.removeEventListener("pagehide",hide)}},[flush,originalId]);
