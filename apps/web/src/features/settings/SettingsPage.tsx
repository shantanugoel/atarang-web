...
import { libraryUsage } from "../../storage/repositories";
import {downloadBackup,restoreBackup} from "../../storage/backup";
import {userMessage} from "../../app/errorText";
import {PERSISTENCE_ADVICE, usePersistenceRequest} from "../../app/persistentStorage";
...
const modelErrors: Record<string, string> = {
  ...
  persistence_denied: "This browser declined to protect Atarang's storage, so a 126 MB download would probably be deleted along with your songs. Bookmark or install Atarang and ask again, keep a backup, or continue at your own risk.",
...
export function SettingsPage() {
  ...
  const {state:persistence,request:requestPersistence}=usePersistenceRequest(refresh);
  // Asking is invisible otherwise: the browser answers in a moment, and a "No"
  // that changes nothing on screen looks like a button that is not wired up.
  const persistenceMessage=persistence==="pending"?"Asking your browser…":persistence==="granted"?"Your browser granted persistent storage. Songs, stems and takes now stay until you delete them.":persistence==="denied"?`Your browser declined the request. ${PERSISTENCE_ADVICE}`:storage&&!storage.persisted?PERSISTENCE_ADVICE:"";
    ...
    <div className={styles.layout}><label className={styles.sectionPicker}>Settings section<select aria-label="Settings section" value={`#${section}`} onChange={event=>{location.hash=event.target.value}}>{SETTING_SECTIONS.map(({id,label})=><option key={id} value={`#${id}`}>{label}</option>)}</select></label><nav aria-label="Settings sections">{SETTING_SECTIONS.map(({id,label,icon:Icon})=><a key={id} href={`#${id}`} aria-current={section===id?"true":undefined}><Icon/>{label}</a>)}</nav>
      <div className={styles.content}>
        <section id="about"><h2>About</h2>
          ...
          <dl className={styles.aboutFacts}><div><dt>Version</dt><dd>{appVersion}</dd></div><div><dt>Author</dt><dd><a href="https://x.com/shantanugoel" target="_blank" rel="noreferrer">Shantanu Goel</a></dd></div></dl>
