import type { ChartLineV1,ChartSegmentV1,UserChartV1 } from "@atarang/contracts";
...
export function transposeChord(value:string,semitones:number,simplify=false){const chord=parseChord(value);if(!chord)return value;const preferFlats=chord.root.includes("b")||chord.bass?.includes("b")===true,quality=simplify?simplifyChord(`${chord.root}${chord.quality}`).slice(chord.root.length):chord.quality;return`${transposeNote(chord.root,semitones,preferFlats)}${quality}${chord.bass?`/${transposeNote(chord.bass,semitones,preferFlats)}`:""}`}
