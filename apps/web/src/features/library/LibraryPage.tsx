...
export function LibraryPage() {
  ...
  return <div className={styles.page} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void importFile(event.dataTransfer.files[0]); }}>
    ...
    <nav className={styles.categories} aria-label="Library category">{(["originals","separated","performances"] as const).map(item=><button key={item} aria-current={category===item?"page":undefined} onClick={()=>setCategory(item)}>{item[0]!.toUpperCase()+item.slice(1)}<span>{item==="performances"?performances.length:item==="separated"?songs.filter(song=>"separated" in song&&song.separated).length:songs.length+1}</span></button>)}</nav>
    <div className={styles.toolbar}><label><MagnifyingGlass/><span className="sr-only">Search library</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${category}`}/></label><span className={styles.sortNote}><ClockCounterClockwise/> Newest first</span></div>
    <section className={styles.table} aria-label="Songs">
      ...
      {category!=="performances"&&visibleSongs.map((song) => <div className={styles.row} key={song.id}>
        ...
        <span className={styles.rowActions}><Link to={`/studio/${song.id}`}>Open</Link><Link to={`/studio/${song.id}?separate=1`}>{"separated" in song&&song.separated?"Separate again":"Separate"}</Link><button aria-label={`Remove ${song.title}`} onClick={() => void removeSong(song.id, song.title)}><Trash/></button></span>
      </div>)}
      {category==="performances"&&visiblePerformances.map(take=>{const song=originalById.get(take.originalId);return <div className={styles.row} key={take.id}><span className={`${styles.art} ${styles.takeArt}`}><Microphone weight="fill"/></span><span className={styles.song}><strong>{song?.title??"Recorded take"}</strong><small>{song?.artist??"Local performance"}</small></span><span>{formatDuration(Math.round(take.manifest.durationFrames/take.manifest.sampleRate*1_000_000))}</span><span>{formatDate(take.createdAt)}</span><span className={styles.asset}><Waveform/>Take</span><span className={styles.rowActions}><Link to={`/studio/${take.originalId}`}>Open takes</Link></span></div>})}
