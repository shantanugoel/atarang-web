...
export function LibraryPage() {
  ...
  return <div className={styles.page} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void importFile(event.dataTransfer.files[0]); }}>
    ...
    <div className={styles.toolbar}><label><MagnifyingGlass/><span className="sr-only">Search library</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${category}`}/></label><button><ClockCounterClockwise/> Recently added</button></div>
