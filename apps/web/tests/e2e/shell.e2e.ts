...
test("cloud operator configuration is session-only and precache excludes private routes",async({page,request})=>{await page.route("http://127.0.0.1:4173/api/v1/capabilities",route=>route.fulfill({json:{cloudEnabled:true}}));await page.goto("/settings");await page.getByLabel("Server address").fill("http://127.0.0.1:4173/path");await page.getByLabel("Deployment key").fill("a".repeat(64));await page.getByRole("button",{name:"Save and test"}).click();await expect(page.getByText("Server reached and accepted your key. Saved for this tab.")).toBeVisible();const stored=await page.evaluate(()=>sessionStorage.getItem("atarang.cloud.configuration"));expect(stored).toContain("http://127.0.0.1:4173");expect(stored).toContain("a".repeat(64));const precache=await request.get("/precache.json");expect(precache.ok()).toBeTruthy();const paths=await precache.json() as string[];expect(paths.some(path=>path.endsWith(".map")||path.startsWith("/api/")||path.startsWith("/models/"))).toBeFalsy()});

test("a rejected deployment key is not saved",async({page})=>{await page.route("http://127.0.0.1:4173/api/v1/capabilities",route=>route.fulfill({status:401,json:{error:{code:"invalid_source"}}}));await page.goto("/settings");await page.getByLabel("Server address").fill("http://127.0.0.1:4173");await page.getByLabel("Deployment key").fill("random-key");await page.getByRole("button",{name:"Save and test"}).click();await expect(page.getByText(/deployment key was rejected/i)).toBeVisible();expect(await page.evaluate(()=>sessionStorage.getItem("atarang.cloud.configuration"))).toBeNull()});
