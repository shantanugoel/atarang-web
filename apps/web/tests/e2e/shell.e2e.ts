...
test("authorized YouTube acquisition exposes server and browser separation choices",async({page,isMobile})=>{let suppliedKey="";await page.addInitScript(()=>localStorage.setItem("atarang.cloud.deploymentKey","a".repeat(64)));await page.route("/api/v1/capabilities",async route=>{suppliedKey=await route.request().headerValue("X-Atarang-Key")??"";await route.fulfill({json:{cloudEnabled:true,youtubeEnabled:true,cpuWorkerAvailable:true}})});await page.goto("/library");const local=page.getByRole("region",{name:"Import local audio"}),youtube=page.getByRole("region",{name:"Fetch from YouTube"}),localAction=local.getByRole("button",{name:"Choose audio",exact:true}),submit=youtube.getByRole("button",{name:"Fetch and separate"});await expect(local).toBeVisible();await expect(youtube).toBeVisible();await expect(localAction).toBeVisible();const[localBox,youtubeBox,localHeading,youtubeHeading]=await Promise.all([local.boundingBox(),youtube.boundingBox(),local.getByRole("heading").boundingBox(),youtube.getByRole("heading").boundingBox()]);expect(Math.round(localBox!.width)).toBe(Math.round(youtubeBox!.width));expect(Math.round(localHeading!.x-localBox!.x)).toBe(Math.round(youtubeHeading!.x-youtubeBox!.x));if(isMobile)expect(youtubeBox!.y).toBeGreaterThan(localBox!.y+localBox!.height);else expect(Math.round(youtubeBox!.y)).toBe(Math.round(localBox!.y));await expect(page.getByLabel("YouTube URL")).toBeVisible();expect(await localAction.evaluate(element=>getComputedStyle(element).backgroundColor)).toBe(await submit.evaluate(element=>getComputedStyle(element).backgroundColor));await page.getByLabel("YouTube URL").fill("https://www.youtube.com/watch?v=Ajxn0PKbv7I");await expect(submit).toBeDisabled();await page.getByLabel("I confirm I am authorized to download and process this content.").check();await expect(submit).toBeEnabled();await page.getByLabel(/Fetch only; separate in this browser/).check();await expect(page.getByRole("button",{name:"Fetch to browser"})).toBeEnabled();expect(suppliedKey).toBe("a".repeat(64))});

// The fetch-only deployment: one container, no worker. Offering "separate on
// server" there is a choice whose only outcome is a job that fails at the end.
test("a server with no worker offers the fetch, not the choice",async({page})=>{await page.addInitScript(()=>localStorage.setItem("atarang.cloud.deploymentKey","a".repeat(64)));await page.route("/api/v1/capabilities",route=>route.fulfill({json:{cloudEnabled:true,youtubeEnabled:true,cpuWorkerAvailable:false,cudaWorkerAvailable:false}}));await page.goto("/library");const youtube=page.getByRole("region",{name:"Fetch from YouTube"});await expect(youtube).toContainText("the stems are made in this browser");await expect(page.getByLabel("YouTube URL")).toBeVisible();await expect(page.getByRole("group",{name:"Separation"})).toHaveCount(0);await page.getByLabel("YouTube URL").fill("https://www.youtube.com/watch?v=Ajxn0PKbv7I");await page.getByLabel("I confirm I am authorized to download and process this content.").check();await expect(youtube.getByRole("button",{name:"Fetch to browser"})).toBeEnabled()});

test("YouTube submission works when randomUUID is unavailable on an HTTP origin",async({page})=>{let idempotencyKey="";await page.addInitScript(()=>{Object.defineProperty(crypto,"randomUUID",{value:undefined,configurable:true});localStorage.setItem("atarang.cloud.deploymentKey","a".repeat(64))});await page.route("/api/v1/capabilities",route=>route.fulfill({json:{cloudEnabled:true,youtubeEnabled:true,cpuWorkerAvailable:true}}));await page.route("/api/v1/jobs",async route=>{idempotencyKey=await route.request().headerValue("Idempotency-Key")??"";await route.fulfill({json:{jobId:"019fef4f-9c77-7a3f-94ca-ef4214a806c1",capabilityToken:"canary-capability",state:"acquiring_youtube"}})});await page.route("/api/v1/jobs/019fef4f-9c77-7a3f-94ca-ef4214a806c1",route=>route.fulfill({json:{state:"failed",stage:"failed",progress:.1}}));await page.goto("/library");await page.getByLabel("YouTube URL").fill("https://www.youtube.com/watch?v=Ajxn0PKbv7I");await page.getByLabel("I confirm I am authorized to download and process this content.").check();await page.getByRole("button",{name:"Fetch and separate"}).click();await expect(page.getByRole("alert")).toContainText("The server could not finish this job");expect(idempotencyKey).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/)});
...
test("chord and settings controls reflow at 320 CSS pixels",async({page,isMobile})=>{test.skip(isMobile,"one 320px reflow pass is enough");
// Answered so detection finds a backend and Settings renders the key form at
// all; a deployment with no server shows the self-hosting notice instead, and
// there would be no cloud controls left to reflow.
await page.route("/api/v1/capabilities",route=>route.fulfill({status:401,json:{error:{code:"invalid_deployment_key"}}}));await page.setViewportSize({width:320,height:800});await page.goto("/library");await page.getByLabel("Choose audio to import").setInputFiles({name:"reflow.wav",mimeType:"audio/wav",buffer:silentWav()});await page.getByRole("button",{name:"Skip for now and just play the song"}).click();await page.getByRole("tab",{name:"Chords"}).click();await page.getByRole("button",{name:"Paste chart"}).click();await page.getByLabel("Paste ChordPro chart").fill("{title: Reflow}\n[Am]One [F]two");await page.getByRole("button",{name:"Add chart"}).click();// Under 760px the chord toolbar is one row that scrolls sideways rather than
// one row per control, so a control sitting outside the viewport is where it is
// meant to be — the thing worth guarding is that every one of them can be
// reached and lands inside the viewport when it is, and that reaching them
// never drags the page itself sideways.
for(const control of[page.getByLabel("Simplify"),...["Transpose down","Transpose up","Decrease capo","Increase capo","Edit","Import","Export","Remove selected chart"].map(name=>page.getByRole("button",{name,exact:true}))]){await expect(control).toBeVisible();await control.scrollIntoViewIfNeeded();expect(await control.evaluate(element=>{const box=element.getBoundingClientRect();return box.left>=0&&box.right<=innerWidth})).toBe(true)}expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBe(320);await page.goto("/settings");await expect(page.getByText("The model has not been downloaded yet.")).toBeVisible();const picker=page.getByLabel("Settings section",{exact:true});await expect(picker).toBeInViewport();await expect(picker).toHaveValue("#about");await page.goto("/settings#privacy");await expect(picker).toHaveValue("#privacy");for(const control of[page.getByRole("button",{name:"Check again"}),page.getByRole("button",{name:"Backup library"}),page.getByRole("button",{name:"Restore backup"}),page.getByLabel("Chord symbol"),page.getByRole("button",{name:"Save voicing"}),page.getByRole("button",{name:/Download browser model/}),page.getByText("Advanced model package",{exact:true}),page.getByLabel("Deployment key"),page.getByRole("button",{name:"Save and test"}),page.getByRole("button",{name:"Forget key"})])await expect(control).toBeVisible();const selects=await page.getByRole("combobox").all();expect(selects).toHaveLength(8);for(const select of selects)await expect(select).toBeVisible();for(const value of["#about","#storage","#audio","#chords","#models","#privacy"]){await picker.selectOption(value);await expect(page).toHaveURL(new RegExp(`${value}$`));expect(await page.locator(value).evaluate(element=>{const box=element.getBoundingClientRect();return box.top>=-1&&box.top<innerHeight})).toBe(true)}await picker.selectOption("#models");await page.goBack();await expect(picker).toHaveValue("#privacy");const routerState=await page.evaluate(()=>history.state);await page.evaluate(()=>{location.hash="#missing"});await expect(page).toHaveURL(/#about$/);await expect(picker).toHaveValue("#about");expect(await page.evaluate(()=>history.state)).toEqual(routerState);expect(await page.locator("#about").evaluate(element=>{const box=element.getBoundingClientRect();return box.top>=-1&&box.top<innerHeight})).toBe(true);expect(await page.evaluate(()=>({width:document.documentElement.scrollWidth,clipped:[...document.querySelectorAll<HTMLElement>('main button,main input,main select,main summary')].filter(element=>element.offsetParent!==null&&!element.closest('nav')).some(element=>{const box=element.getBoundingClientRect();return box.left<0||box.right>innerWidth})}))).toEqual({width:320,clipped:false})});
...
test("Settings opens on About, and the side navigation marks where you are",async({page,isMobile})=>{
  await page.goto("/settings");
  const about=page.locator("#about");
  await expect(about.getByRole("heading",{name:"About"})).toBeInViewport();
  await expect(about).toContainText("Everything happens in this browser.");
  // A version nobody can read off the screen is a support conversation that
  // starts with "which build are you on?" and has no answer.
  // Version and build together, which is what the build stamps and what a
  // support conversation actually needs.
  await expect(about.getByText(/^\d+\.\d+\.\d+ \[[0-9a-f]{7}(-dirty)?\]$/)).toBeVisible();
  await expect(about).toContainText("Shantanu Goel");
  await expect(about.getByRole("link",{name:/GitHub/})).toHaveAttribute("href","https://github.com/shantanugoel/atarang-web");
  // The side navigation belongs to the wide layout; the compact one has a
  // picker instead, which the reflow test already walks.
  if(isMobile)return;
  // The highlight used to be hardcoded onto Storage, so it said Storage from
  // every section including this one.
  const navigation=page.getByRole("navigation",{name:"Settings sections"});
  await expect(navigation.getByRole("link",{name:"About"})).toHaveAttribute("aria-current","true");
  await navigation.getByRole("link",{name:"Models"}).click();
  await expect(navigation.getByRole("link",{name:"Models"})).toHaveAttribute("aria-current","true");
  await expect(navigation.getByRole("link",{name:"About"})).not.toHaveAttribute("aria-current","true");
});

test("the operator key outlives the tab and precache excludes private routes",async({page,request})=>{await page.route("/api/v1/capabilities",route=>route.fulfill({json:{cloudEnabled:true}}));await page.goto("/settings");await page.getByLabel("Deployment key").fill("a".repeat(64));await page.getByRole("button",{name:"Save and test"}).click();await expect(page.getByText("Server reached and accepted your key. Saved in this browser.")).toBeVisible();expect(await page.evaluate(()=>localStorage.getItem("atarang.cloud.deploymentKey"))).toBe("a".repeat(64));
// The point of the change: a reload must not ask for the key again.
await page.reload();await expect(page.getByText("Saved in this browser.")).toBeVisible();const precache=await request.get("/precache.json");expect(precache.ok()).toBeTruthy();const paths=await precache.json() as string[];expect(paths.some(path=>path.endsWith(".map")||path.startsWith("/api/")||path.startsWith("/models/"))).toBeFalsy()});

test("a rejected deployment key is not saved",async({page})=>{await page.route("/api/v1/capabilities",route=>route.fulfill({status:401,json:{error:{code:"invalid_source"}}}));await page.goto("/settings");await page.getByLabel("Deployment key").fill("random-key");await page.getByRole("button",{name:"Save and test"}).click();await expect(page.getByText(/deployment key was rejected/i)).toBeVisible();expect(await page.evaluate(()=>localStorage.getItem("atarang.cloud.deploymentKey"))).toBeNull()});
...
test("originals expose an explicit separation action that opens the chooser",async({page})=>{const errors:string[]=[];page.on("console",message=>{if(message.type()==="error")errors.push(message.text())});page.on("pageerror",error=>errors.push(error.message));await page.goto("/library");const chooser=page.waitForEvent("filechooser");await page.getByRole("button",{name:"Choose audio",exact:true}).click();await(await chooser).setFiles({name:"route-check.wav",mimeType:"audio/wav",buffer:silentWav()});await expect(page).toHaveURL(/\/studio\//);await page.getByRole("button",{name:"Skip for now and just play the song"}).click();await expect(page).toHaveTitle("Route Check — Atarang");await page.getByRole("link",{name:"Library"}).click();const separate=page.getByRole("link",{name:"Separate",exact:true});await expect(separate).toBeVisible();await separate.click();const dialog=page.getByRole("dialog",{name:"Separate this song"});await expect(dialog).toBeVisible();await expect(dialog.getByText("Local on this device")).toBeVisible();// With no model downloaded the local route offers the download rather than a
// disabled button that explains nothing.
await expect(dialog.getByRole("link",{name:"Download model"})).toBeVisible();await expect(dialog.getByText("Audio is never uploaded automatically.")).toBeVisible();await expect(dialog.getByRole("button",{name:"Import package"})).toBeEnabled();expect(errors).toEqual([])});
...
test("an installed WebGPU model works without a benchmark and reports inference failure persistently",async({page})=>{
  ...
  await page.reload();
  await expect(page.getByText("Ready · speed not measured")).toBeVisible();
  await expect(page.getByText(/installed and ready/)).toBeVisible();
  await page.getByRole("button",{name:"Test this device's speed (optional)"}).click();
  await expect(page.getByText(/Running optional speed test… 42%/)).toBeVisible();
  await page.getByRole("link",{name:"Library"}).click();
  await expect(page.getByRole("heading",{name:"Library"})).toBeVisible();
  await page.getByRole("navigation",{name:"Primary navigation"}).getByRole("link",{name:"Settings"}).click();
  await expect(page.getByText(/Running optional speed test… 42%/)).toBeVisible();
  await expect(page.getByText("Ready · about 3 minutes for a 4-minute song")).toBeVisible();
  await page.getByRole("link",{name:"Library"}).click();
...
test("a running model test blocks concurrent separation and can be cancelled",async({page})=>{
  ...
  await page.reload();
  await page.getByRole("button",{name:"Test this device's speed (optional)"}).click();
  await expect(page.getByText(/Running optional speed test/)).toBeVisible();
  await page.getByRole("link",{name:"Library"}).click();
  ...
  await expect(page.getByRole("button",{name:"Test this device's speed (optional)"})).toBeEnabled();
...
test("a chart stored before simplification had levels still backs up and opens",async({page,isMobile})=>{
  const errors:string[]=[];page.on("console",message=>{if(message.type()==="error")errors.push(message.text())});page.on("pageerror",error=>errors.push(error.message));
  await page.goto("/library");
  await page.getByLabel("Choose audio to import").setInputFiles({name:"legacy-chart.wav",mimeType:"audio/wav",buffer:silentWav()});
  await page.getByRole("button",{name:"Skip for now and just play the song"}).click();
  if(isMobile)await page.getByRole("button",{name:"Song",exact:true}).click();
  await page.getByRole("tab",{name:"Chords"}).click();
  await page.getByRole("button",{name:"Paste chart"}).click();
  await page.getByLabel("Paste ChordPro chart").fill("{title: Legacy}\n[Am7]Old [Cmaj9]shape");
  await page.getByRole("button",{name:"Add chart"}).click();
  await expect(page.getByRole("button",{name:"Show Am7 diagram"})).toBeVisible();
  // Rewritten as it was stored before this build: `simplify`, no `complexity`.
  // Nothing rewrites a chart until the user edits it, so this is the state most
  // libraries are actually in on the first run after the upgrade.
  const chartId=await page.evaluate(async()=>{
    const db=await new Promise<IDBDatabase>((resolve,reject)=>{const request=indexedDB.open("atarang");request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error)});
    const records=await new Promise<{id:string;document:Record<string,unknown>}[]>((resolve,reject)=>{const query=db.transaction("charts").objectStore("charts").getAll();query.onsuccess=()=>resolve(query.result);query.onerror=()=>reject(query.error)});
    const record=records[0]!,{complexity,...document}=record.document;
    await new Promise<void>((resolve,reject)=>{const transaction=db.transaction("charts","readwrite");transaction.objectStore("charts").put({...record,document:{...document,simplify:complexity!=="full"}});transaction.oncomplete=()=>resolve();transaction.onerror=()=>reject(transaction.error)});
    db.close();
    return record.id;
  });
  ...
  await page.getByRole("button",{name:"Backup library"}).click();
  await expect(page.getByText("Backup ready.")).toBeVisible();
  const backupPath=await(await downloadPromise).path();
  // And the same chart restores, rather than taking the whole backup down.
