...
export interface BeatGridV1 {schema:"atarang.beats/1";originalId:string;revision:number;algorithmVersion:"atarang-spectral-flux/1";bpm:number;reliability:number;reliable:boolean;userEdited:boolean;beats:BeatV1[];updatedAt:string}
export interface ChordSegmentV1{startTimeUs:number;endTimeUs:number;chord:string;confidence:number}
export interface ChordAnalysisV1{schema:"atarang.chords/1";originalId:string;revision:number;algorithmVersion:"atarang-chroma/1";key:string|null;confidence:number;segments:ChordSegmentV1[];updatedAt:string}
...
export interface PerformanceManifestV1 {schema:"atarang.performance/1";performanceId:string;originalId:string;revision:number;startedAt:string;endedAt:string;sampleRate:number;channels:2;durationFrames:number;mic:PerformanceAssetV1;backing:PerformanceAssetV1;inputOffsetUs:number;deviceSettings:{sampleRate?:number;channelCount?:number;echoCancellation?:boolean;noiseSuppression?:boolean;autoGainControl?:boolean};edit:{trimStartUs:number;trimEndUs:number;fadeInUs:number;fadeOutUs:number};updatedAt:string}
...
export function beatGridErrors(value:unknown):string[]{if(!value||typeof value!=="object")return["beat grid must be an object"];const grid=value as Partial<BeatGridV1>,errors:string[]=[];if(grid.schema!=="atarang.beats/1"||grid.algorithmVersion!=="atarang-spectral-flux/1")errors.push("beat schema or algorithm is invalid");if(!grid.originalId||!UUID.test(grid.originalId))errors.push("originalId must be a UUID");if(!Number.isInteger(grid.revision)||grid.revision!<0)errors.push("revision is invalid");if(typeof grid.bpm!=="number"||grid.bpm<30||grid.bpm>300)errors.push("bpm must be between 30 and 300");if(typeof grid.reliability!=="number"||grid.reliability<0||grid.reliability>1||typeof grid.reliable!=="boolean"||typeof grid.userEdited!=="boolean")errors.push("reliability is invalid");if(!Array.isArray(grid.beats)||grid.beats.some((beat,index)=>!Number.isSafeInteger(beat.timeUs)||beat.timeUs<0||beat.beatInBar!==index%4+1||beat.downbeat!==(beat.beatInBar===1)))errors.push("beats are invalid");if(!grid.updatedAt||Number.isNaN(Date.parse(grid.updatedAt)))errors.push("updatedAt must be an ISO date-time");return errors}
